...
if ($user['step'] == "createusertest" || preg_match('/locationtest_(.*)/', $datain, $dataget) || ($text == $textbotlang['textbot']['userTest'] || $datain == "usertestbtn" || $text == "usertest")) {
    ...
    if ($setting['categoryhelp'] == "1") {
        ...
        $helpNameIconsMap = [];
        if (!empty($setting['helpNameIcons'])) {
            $decodedNameIcons = json_decode($setting['helpNameIcons'], true);
            if (is_array($decodedNameIcons)) {
                $helpNameIconsMap = $decodedNameIcons;
            }
        }
        foreach ($helplist as $result) {
            $nameButton = ['text' => $result['name_os'], 'callback_data' => "helpos_{$result['id']}"];
            if (isset($helpNameIconsMap[$result['name_os']]) && $helpNameIconsMap[$result['name_os']] !== '') {
                $nameButton['icon_custom_emoji_id'] = (string) $helpNameIconsMap[$result['name_os']];
            }
