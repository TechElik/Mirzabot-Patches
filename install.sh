...
function self_update_script() {
    ...
    local URL="https://raw.githubusercontent.com/techelik/mirzabot-patches/main/install.sh"
...
GIT_REPO="techelik/mirzabot-patches"
...
# Choose which source to download - ALWAYS uses the latest release
# Sets globals: SRC_ZIP_URL, SRC_LABEL
# Returns: 0 = chosen, 1 = error
choose_source() {
    SRC_ZIP_URL=""; SRC_LABEL=""
    
    # Always use the latest stable release from GitHub
    ...
    if [ -n "$l" ]; then 
        SRC_ZIP_URL="${tagbase}/${l}.zip"
        SRC_LABEL="Release ${l}"
        echo -e "  ${C_OK}●${CR} ${C_OK}Using latest version: ${l}${CR}"
        return 0
    else
        # Fallback to main branch if version detection fails
        echo -e "  ${C_WARN}!${CR} ${C_WARN}Could not detect latest release; using beta (main branch).${CR}"
        SRC_ZIP_URL="$beta"
        SRC_LABEL="Beta (main)"
        return 0
    fi
...
function show_menu() {
    ...
    _mi "1" "Install Mirza"
    _mi "2" "Remove Mirza"
    _mi "3" "Renew SSL certificate"
    _mi "4" "Backup Database"
    _mi "5" "Import Database ${C_WARN}(Beta)${CR}"
...
function install_bot() {
    rm -f /tmp/.mirza_latest_version   # اضافه شده
    ...
    if ! phase_done DEPS; then
        # Choose which version to install (ALWAYS latest, no user interaction)
        echo ""
        choose_source
        local _rc=$?
...
EOF
    ...
    if ! phase_done WEBHOOK; then
        ...
        run_step "Tuning PHP-FPM for stability (timeout + worker limits)" \
            "if [ -f '$FPM_POOL' ]; then \
                grep -q '^request_terminate_timeout' '$FPM_POOL' && sed -i 's/^request_terminate_timeout.*/request_terminate_timeout = 30s/' '$FPM_POOL' || echo 'request_terminate_timeout = 30s' >> '$FPM_POOL'; \
                sed -i 's/^pm.max_children = .*/pm.max_children = 12/' '$FPM_POOL'; \
                systemctl restart php${PHP_VER}-fpm; \
            fi" \
            || { show_step_error; install_pause "Tuning PHP-FPM"; }
...
print_usage() {
  ...
  Examples:
    mirza install --name myvpnbot --token 123:ABC --admin 111 --domain bot.example.com
...
process_arguments() {
    ...
    case "$1" in
        install|remove|renew|backup|import|menu) cmd="$1"; shift ;;
