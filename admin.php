...
elseif ($datain == "systemsms") {
    ...
    step('home', $from_id);
} elseif ($text == "🎨 آیکون محصول" && $adminrulecheck['rule'] == "administrator") {
    $prow = select("product", "*", "id", $user['Processing_value'], "select");
    if (!$prow) {
        sendmessage($from_id, "این محصول پیدا نشد.", $change_product, 'HTML');
        return;
    }
    savedata("clear", "producticon_name", $prow['name_product']);
    step('changeprodicon', $from_id);
    sendmessage($from_id, "همون ایموجی پرمیوم متحرکی که می‌خوای برای محصول «" . $prow['name_product'] . "» استفاده بشه رو مستقیم اینجا بفرست (یا فوروارد کن).", $backadmin, 'HTML');
} elseif ($user['step'] == "changeprodicon") {
    $userdata = json_decode((string) ($user['Processing_value'] ?? ''), true);
    $pname = $userdata['producticon_name'] ?? '';
    ...
    if (!empty($setting_now['productIcons'])) {
        $decoded = json_decode($setting_now['productIcons'], true);
        if (is_array($decoded)) {
            $prodicons = $decoded;
        }
    }
    $prodicons[$pname] = $foundid;
    update("setting", "productIcons", json_encode($prodicons, JSON_UNESCAPED_UNICODE), null, null);
    step('home', $from_id);
    sendmessage($from_id, "✅ آیکون محصول «" . $pname . "» ذخیره شد.", $change_product, 'HTML');
} elseif ($text == "🎨 آیکون دسته‌بندی فروشگاه" && $adminrulecheck['rule'] == "administrator") {
    $catrows = select("category", "*", null, null, "fetchAll");
    $catnames = [];
    foreach ($catrows as $r) {
        if (!empty($r['remark']) && !in_array($r['remark'], $catnames)) {
            $catnames[] = $r['remark'];
        }
    }
    if (empty($catnames)) {
        sendmessage($from_id, "هیچ دسته‌بندی‌ای پیدا نشد.", $keyboard_Category_manage, 'HTML');
        return;
    }
    $catkeyboard = ['inline_keyboard' => []];
    foreach ($catnames as $cn) {
        $catkeyboard['inline_keyboard'][] = [
            ['text' => $cn, 'callback_data' => "shopcaticon_select:" . dechex(crc32($cn))]
        ];
    }
    $catkeyboard['inline_keyboard'][] = [['text' => '❌ بستن', 'callback_data' => 'bt_close']];
    savedata("clear", "shopcaticon_map", $catnames);
    sendmessage($from_id, "کدوم دسته‌بندی رو می‌خوای آیکون بدی؟", json_encode($catkeyboard), 'HTML');
} elseif (preg_match('/^shopcaticon_select:([0-9a-f]+)$/', $datain, $dataget)) {
    $userdata = select("user", "*", "id", $from_id, "select");
    $savedmap = json_decode((string) ($userdata['Processing_value'] ?? ''), true);
    $catnames = $savedmap['shopcaticon_map'] ?? [];
    $matched = null;
    foreach ($catnames as $cn) {
        if (dechex(crc32($cn)) == $dataget[1]) {
            $matched = $cn;
            break;
        }
    }
    if ($matched === null) {
        sendmessage($from_id, "این دسته‌بندی پیدا نشد، دوباره از منو امتحان کن.", null, 'HTML');
        return;
    }
    savedata("save", "shopcaticon_selected", $matched);
    step("get_shop_category_icon_emoji", $from_id);
    sendmessage($from_id, "همون ایموجی پرمیوم متحرکی که می‌خوای برای دسته‌بندی «" . $matched . "» استفاده بشه رو مستقیم اینجا بفرست (یا فوروارد کن).", null, 'HTML');
} elseif ($user['step'] == "get_shop_category_icon_emoji") {
    $userdata = json_decode((string) ($user['Processing_value'] ?? ''), true);
    $catname = $userdata['shopcaticon_selected'] ?? '';
    $msgentities = $update['message']['entities'] ?? [];
    $foundid = null;
    foreach ($msgentities as $ent) {
        if (($ent['type'] ?? '') == 'custom_emoji' && !empty($ent['custom_emoji_id'])) {
    ...
    if (!empty($setting_now['shopCategoryIcons'])) {
        $decoded = json_decode($setting_now['shopCategoryIcons'], true);
        if (is_array($decoded)) {
            $shopicons = $decoded;
        }
    }
    $shopicons[$catname] = $foundid;
    update("setting", "shopCategoryIcons", json_encode($shopicons, JSON_UNESCAPED_UNICODE), null, null);
    step('home', $from_id);
    sendmessage($from_id, "✅ آیکون دسته‌بندی «" . $catname . "» ذخیره شد.", $keyboard_Category_manage, 'HTML');
...
if ($datain == "settimecornremove" && $adminrulecheck['rule'] == "administrator") {
    ...
    sendmessage($from_id, $textbotlang['Admin']['Help']['descUpdated'], $helpedit, 'HTML');
    step('home', $from_id);
} elseif ($text == "🎨 آیکون دسته‌بندی" && $adminrulecheck['rule'] == "administrator") {
    $helprow = select("help", "*", "name_os", $user['Processing_value'], "select");
    if (!$helprow || empty($helprow['category'])) {
        sendmessage($from_id, "این آموزش دسته‌بندی نداره یا پیدا نشد.", $helpedit, 'HTML');
        return;
    }
    savedata("clear", "helpcaticon_category", $helprow['category']);
    step('changehelpcategoryicon', $from_id);
    sendmessage($from_id, "همون ایموجی پرمیوم متحرکی که می‌خوای برای دسته‌بندی «" . $helprow['category'] . "» استفاده بشه رو مستقیم اینجا بفرست (یا فوروارد کن).", $backadmin, 'HTML');
} elseif ($user['step'] == "changehelpcategoryicon") {
    $userdata = json_decode((string) ($user['Processing_value'] ?? ''), true);
    $catname = $userdata['helpcaticon_category'] ?? '';
    $msgentities = $update['message']['entities'] ?? [];
    $foundid = null;
    foreach ($msgentities as $ent) {
        if (($ent['type'] ?? '') == 'custom_emoji' && !empty($ent['custom_emoji_id'])) {
    ...
    if (!empty($setting_now['helpCategoryIcons'])) {
        $decoded = json_decode($setting_now['helpCategoryIcons'], true);
        if (is_array($decoded)) {
            $icons = $decoded;
        }
    }
    $icons[$catname] = $foundid;
    update("setting", "helpCategoryIcons", json_encode($icons, JSON_UNESCAPED_UNICODE), null, null);
    step('home', $from_id);
    sendmessage($from_id, "✅ آیکون دسته‌بندی «" . $catname . "» ذخیره شد.", $helpedit, 'HTML');
} elseif ($text == "🎨 آیکون آموزش" && $adminrulecheck['rule'] == "administrator") {
    $helprow = select("help", "*", "name_os", $user['Processing_value'], "select");
    if (!$helprow) {
        sendmessage($from_id, "این آموزش پیدا نشد.", $helpedit, 'HTML');
        return;
    }
    savedata("clear", "helpnameicon_name", $helprow['name_os']);
    step('changehelpnameicon', $from_id);
    sendmessage($from_id, "همون ایموجی پرمیوم متحرکی که می‌خوای برای آموزش «" . $helprow['name_os'] . "» استفاده بشه رو مستقیم اینجا بفرست (یا فوروارد کن).", $backadmin, 'HTML');
} elseif ($user['step'] == "changehelpnameicon") {
    $userdata = json_decode((string) ($user['Processing_value'] ?? ''), true);
    $namekey = $userdata['helpnameicon_name'] ?? '';
    $msgentities = $update['message']['entities'] ?? [];
    $foundid = null;
    foreach ($msgentities as $ent) {
        if (($ent['type'] ?? '') == 'custom_emoji' && !empty($ent['custom_emoji_id'])) {
    ...
    if (!empty($setting_now['helpNameIcons'])) {
        $decoded = json_decode($setting_now['helpNameIcons'], true);
        if (is_array($decoded)) {
            $nameicons = $decoded;
        }
    }
    $nameicons[$namekey] = $foundid;
    update("setting", "helpNameIcons", json_encode($nameicons, JSON_UNESCAPED_UNICODE), null, null);
    step('home', $from_id);
    sendmessage($from_id, "✅ آیکون آموزش «" . $namekey . "» ذخیره شد.", $helpedit, 'HTML');
...
if ($datain == "settimecornday" && $adminrulecheck['rule'] == "administrator") {
...
} elseif ($datain == "btnicon_menu") {
    $btnIconItems = [
        'textbot.sell' => 'دکمه: خرید اشتراک',
        'textbot.purchasedServices' => 'دکمه: سرویس‌های من',
        'textbot.extend' => 'دکمه: تمدید سرویس',
        'textbot.userTest' => 'دکمه: اکانت تست',
        'textbot.accountWallet' => 'دکمه: کیف پول و شارژ',
        'textbot.addBalance' => 'دکمه: افزایش موجودی',
        'textbot.tariffList' => 'دکمه: تعرفه‌ها',
        'textbot.support' => 'دکمه: پشتیبانی',
        'textbot.help' => 'دکمه: آموزش',
        'textbot.affiliates' => 'دکمه: زیرمجموعه‌گیری',
        'textbot.discount' => 'دکمه: کد هدیه',
    ...
    foreach ($btnIconItems as $itemKey => $label) {
        $btnkeyboard['inline_keyboard'][] = [
            ['text' => $label, 'callback_data' => "btnicon_select:" . dechex(crc32($itemKey))]
        ];
    }
    $btnkeyboard['inline_keyboard'][] = [['text' => '❌ بستن', 'callback_data' => 'bt_close']];
    savedata("clear", "btnicon_map", array_keys($btnIconItems));
    sendmessage($from_id, "کدوم دکمه رو می‌خوای آیکون بدی؟ (این‌ها همیشه قابل انتخابن، حتی اگه الان توی کیبورد فعال نباشن)", json_encode($btnkeyboard), 'HTML');
} elseif (preg_match('/^btnicon_select:([0-9a-f]+)$/', $datain, $dataget)) {
    $userdata = select("user", "*", "id", $from_id, "select");
    $savedmap = json_decode((string) ($userdata['Processing_value'] ?? ''), true);
    $btnmap = $savedmap['btnicon_map'] ?? [];
    $matched = null;
    foreach ($btnmap as $ph) {
        if (dechex(crc32($ph)) == $dataget[1]) {
            $matched = $ph;
            break;
        }
    }
    if ($matched === null) {
        sendmessage($from_id, "این دکمه پیدا نشد، دوباره از منو امتحان کن.", null, 'HTML');
        return;
    }
    savedata("save", "btnicon_selected", $matched);
    step("get_button_icon_emoji", $from_id);
    sendmessage($from_id, "خب حالا همون ایموجی پرمیوم متحرکی که می‌خوای برای این دکمه استفاده بشه رو مستقیم اینجا بفرست (یا فوروارد کن).", null, 'HTML');
} elseif ($user['step'] == "get_button_icon_emoji") {
    $userdata = json_decode((string) ($user['Processing_value'] ?? ''), true);
    $btnkey = $userdata['btnicon_selected'] ?? '';
    $msgentities = $update['message']['entities'] ?? [];
    $foundid = null;
    foreach ($msgentities as $ent) {
        if (($ent['type'] ?? '') == 'custom_emoji' && !empty($ent['custom_emoji_id'])) {
    ...
    if (!empty($setting_now['buttonIcons'])) {
        $decoded = json_decode($setting_now['buttonIcons'], true);
        if (is_array($decoded)) {
            $btnicons = $decoded;
        }
    }
    $btnicons[$btnkey] = $foundid;
    update("setting", "buttonIcons", json_encode($btnicons, JSON_UNESCAPED_UNICODE), null, null);
    step('home', $from_id);
    sendmessage($from_id, "✅ ذخیره شد. این دکمه از این به بعد این آیکون رو داره (حتی وقتی فعال بشه).", $keyboardadmin, 'HTML');
}
