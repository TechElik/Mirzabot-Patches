...
if (!empty($setting['buttonIcons'])) {
    $decodedBtnIcons = json_decode($setting['buttonIcons'], true);
    if (is_array($decodedBtnIcons)) {
        $buttonIconsMap = $decodedBtnIcons;
    }
}
if (!empty($keyboardRows)) {
    ...
    foreach ($keyboardRows as $kb_r => $kb_row) {
        ...
        foreach ($kb_row as $kb_c => $kb_btn) {
            ...
            if (is_array($kb_btn) && isset($kb_btn['text']) && isset($btnIconMap[$kb_btn['text']])) {
                $itemKey = $btnIconMap[$kb_btn['text']];
                if (isset($buttonIconsMap[$itemKey]) && $buttonIconsMap[$itemKey] !== '') {
                    $keyboardRows[$kb_r][$kb_c]['icon_custom_emoji_id'] = (string) $buttonIconsMap[$itemKey];
                }
            }
        }
    }
...
$keyboardPanel_btn_discount = ['text' => $textbotlang['textbot']['discount'], 'callback_data' => "Discount"];
$keyboardPanel_btn_addbalance = ['text' => $textbotlang['textbot']['addBalance'], 'callback_data' => "Add_Balance"];
if (isset($buttonIconsMap['textbot.discount']) && $buttonIconsMap['textbot.discount'] !== '') {
    $keyboardPanel_btn_discount['icon_custom_emoji_id'] = (string) $buttonIconsMap['textbot.discount'];
}
if (isset($buttonIconsMap['textbot.addBalance']) && $buttonIconsMap['textbot.addBalance'] !== '') {
    $keyboardPanel_btn_addbalance['icon_custom_emoji_id'] = (string) $buttonIconsMap['textbot.addBalance'];
}
$keyboardPanel = json_encode([
    'inline_keyboard' => [
        [
            $keyboardPanel_btn_discount,
            $keyboardPanel_btn_addbalance
        ],
        [
...
$helpCategoryMap = [];
$helpCategoryIcons = [];
if (!empty($setting['helpCategoryIcons'])) {
    $decodedIcons = json_decode($setting['helpCategoryIcons'], true);
    if (is_array($decodedIcons)) {
        $helpCategoryIcons = $decodedIcons;
    }
}
foreach ($helpRows as $result) {
    ...
    $categoryButton = ['text' => $result['category'], 'callback_data' => "helpctg_{$catId}"];
    if (isset($helpCategoryIcons[$result['category']]) && $helpCategoryIcons[$result['category']] !== '') {
        $categoryButton['icon_custom_emoji_id'] = (string) $helpCategoryIcons[$result['category']];
    }
...
$faqBtn = ['text' => $textbotlang['textbot']['faq'], 'callback_data' => "fqQuestions"];
if (isset($buttonIconsMap['textbot.faq']) && $buttonIconsMap['textbot.faq'] !== '') {
    $faqBtn['icon_custom_emoji_id'] = (string) $buttonIconsMap['textbot.faq'];
}
$supportMsgBtn_url = ['text' => $textbotlang['keyboard']['sendMessageToSupport'], 'url' => "https://t.me/{$setting['id_support']}"];
$supportMsgBtn_callback = ['text' => $textbotlang['keyboard']['sendMessageToSupport'], 'callback_data' => "support"];
if (isset($buttonIconsMap['keyboard.sendMessageToSupport']) && $buttonIconsMap['keyboard.sendMessageToSupport'] !== '') {
    $supportMsgBtn_url['icon_custom_emoji_id'] = (string) $buttonIconsMap['keyboard.sendMessageToSupport'];
    $supportMsgBtn_callback['icon_custom_emoji_id'] = (string) $buttonIconsMap['keyboard.sendMessageToSupport'];
}
...
$helpedit = json_encode([
    'keyboard' => [
        ...
        [['text' => '🎨 آیکون دسته‌بندی'], ['text' => '🎨 آیکون آموزش']],
        [['text' => $textbotlang['Admin']['backAdminBtn']], ['text' => $textbotlang['Admin']['backMenuBtn']]]
    ],
    'resize_keyboard' => true
]);
...
function KeyboardProduct($location, $query, $pricediscount, $datakeyboard, $statuscustom = false, $backuser = "backuser", $valuetow = null, $customvolume = "customsellvolume")
{
    ...
    $productIconsMap = [];
    if (!empty($setting['productIcons'])) {
        $decodedProductIcons = json_decode($setting['productIcons'], true);
        if (is_array($decodedProductIcons)) {
            $productIconsMap = $decodedProductIcons;
        }
    }
    ...
    while ($result = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $originalProductName = $result['name_product'];
        ...
        $productButton = ['text' => $result['name_product'], 'callback_data' => "{$datakeyboard}{$result['code_product']}{$valuetow}"];
        if (isset($productIconsMap[$originalProductName]) && $productIconsMap[$originalProductName] !== '') {
            $productButton['icon_custom_emoji_id'] = (string) $productIconsMap[$originalProductName];
        }
...
function KeyboardCategory($location, $agent, $backuser = "backuser")
{
    ...
    $shopCategoryIconsMap = [];
    if (!empty($setting['shopCategoryIcons'])) {
        $decodedShopCatIcons = json_decode($setting['shopCategoryIcons'], true);
        if (is_array($decodedShopCatIcons)) {
            $shopCategoryIconsMap = $decodedShopCatIcons;
        }
    }
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        if (empty($categoryCounts[mb_strtolower(trim((string) $row['remark']), 'UTF-8')]))
            continue;
        $catButton = ['text' => $row['remark'], 'callback_data' => "categorynames_" . $row['id']];
        if (isset($shopCategoryIconsMap[$row['remark']]) && $shopCategoryIconsMap[$row['remark']] !== '') {
            $catButton['icon_custom_emoji_id'] = (string) $shopCategoryIconsMap[$row['remark']];
        }
        $list_category['inline_keyboard'][] = [$catButton];
    }
    $list_category['inline_keyboard'][] = [
...
function keyboard_list_text($lang)
{
    ...
    $keyboard_text['inline_keyboard'][] = [['text' => '🎨 آیکون دکمه‌های کیبورد', 'callback_data' => 'btnicon_menu']];
